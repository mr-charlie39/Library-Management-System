...
body {
	...
	color: var(--ink);
	background: radial-gradient(circle at 10% 16%, rgba(127, 70, 192, 0.33),
		transparent 34%),
		radial-gradient(circle at 86% 84%, rgba(44, 163, 136, 0.22),
		transparent 36%),
...
.entry-loader {
	...
	display: grid;
	place-items: center;
	background: radial-gradient(circle at 20% 20%, rgba(127, 70, 192, 0.2),
		transparent 33%), rgba(9, 6, 14, 0.88);
	transition: opacity 560ms var(--ease-smooth), visibility 560ms
		var(--ease-smooth);
}
...
.loader-card {
	...
	background: linear-gradient(160deg, rgba(36, 21, 52, 0.93),
		rgba(42, 31, 70, 0.92));
	padding: 1.2rem;
	text-align: center;
...
.loader-icon {
	...
	position: relative;
	background: linear-gradient(145deg, rgba(127, 70, 192, 0.32),
		rgba(44, 163, 136, 0.22));
}
...
.loader-bars span {
	...
	background: linear-gradient(90deg, rgba(191, 156, 255, 0.88),
		rgba(44, 163, 136, 0.68));
...
.orb-a {
	...
	left: -100px;
	top: 20%;
	background: radial-gradient(circle, rgba(127, 70, 192, 0.58),
		rgba(127, 70, 192, 0));
}
...
.orb-b {
	...
	right: -90px;
	top: 60%;
	background: radial-gradient(circle, rgba(44, 163, 136, 0.5),
		rgba(44, 163, 136, 0));
	animation-delay: 4s;
}
...
.orb-c {
	...
	right: 28%;
	top: 8%;
	background: radial-gradient(circle, rgba(201, 133, 71, 0.5),
		rgba(201, 133, 71, 0));
	animation-delay: 7s;
}
...
.app-shell {
	...
	filter: blur(4px);
	transition: opacity 760ms var(--ease-smooth), transform 760ms
		var(--ease-smooth), filter 760ms var(--ease-smooth);
...
.sidebar {
	border-radius: 22px;
	background: linear-gradient(180deg, rgba(32, 19, 45, 0.95),
		rgba(24, 18, 34, 0.93));
...
.brand-mark {
	...
	place-items: center;
	font-size: 1.1rem;
	background: linear-gradient(145deg, rgba(127, 70, 192, 0.3),
		rgba(44, 163, 136, 0.2));
...
.nav-link {
	...
	transition: background-color 260ms var(--ease-gentle), color 260ms
		var(--ease-gentle), transform 260ms var(--ease-gentle);
}
...
.main {
	border-radius: 22px;
	background: linear-gradient(180deg, rgba(251, 247, 255, 0.96),
		rgba(242, 234, 250, 0.96));
	border: 1px solid rgba(227, 216, 239, 0.95);
	box-shadow: 0 24px 56px rgba(11, 8, 18, 0.34), 0 0 0 1px
...
.topbar {
	...
	color: #f9f4ff;
	background: linear-gradient(110deg, #2b1340, #61379a 46%, #2d7f93),
		repeating-linear-gradient(45deg, rgba(255, 255, 255, 0.08) 0,
		rgba(255, 255, 255, 0.08) 6px, rgba(255, 255, 255, 0) 6px,
...
.ghost-link {
	...
	transition: transform 240ms var(--ease-gentle), background-color 240ms
		var(--ease-gentle);
	display: inline-flex;
	align-items: center;
	gap: 0.45rem;
...
.reveal {
	...
	transition: transform 680ms var(--ease-smooth), opacity 680ms
		var(--ease-smooth);
...
.form-card::before {
	...
	top: -88px;
	border-radius: 50%;
	background: radial-gradient(circle, rgba(127, 70, 192, 0.2),
		rgba(127, 70, 192, 0));
...
.form-card::after {
	...
	bottom: -98px;
	border-radius: 50%;
	background: radial-gradient(circle, rgba(44, 163, 136, 0.17),
		rgba(44, 163, 136, 0));
	animation: aura-shift 13s var(--ease-gentle) reverse infinite;
}

.form-card.no-top-orb::before {
  display: none;
}
...
.form-section::before {
	...
	position: absolute;
	inset: 0;
	background: radial-gradient(circle at 100% 0%, rgba(127, 70, 192, 0.05),
		transparent 60%),
		radial-gradient(circle at 0% 100%, rgba(44, 163, 136, 0.04),
		transparent 70%);
	pointer-events: none;
	z-index: 0;
}
...
.field input, .field select, .field textarea {
	...
	box-shadow: 0 2px 0 rgba(127, 70, 192, 0.06), inset 0 1px 0
		rgba(255, 255, 255, 0.8);
	transition: border-color 280ms var(--ease-smooth), box-shadow 280ms
		var(--ease-smooth), transform 280ms var(--ease-smooth),
...
.field input:focus, .field select:focus, .field textarea:focus {
	...
	box-shadow: 0 0 0 4px rgba(127, 70, 192, 0.2), 0 12px 28px
		rgba(96, 54, 144, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.9);
	transform: translateY(-2px);
}

.field input:hover, .field select:hover, .field textarea:hover {
	border-color: #d0c4e8;
	box-shadow: 0 4px 12px rgba(127, 70, 192, 0.12), inset 0 1px 0
		rgba(255, 255, 255, 0.8);
}
...
.role-card input:checked+.role-content {
	...
	background: linear-gradient(135deg, rgba(127, 70, 192, 0.16),
		rgba(44, 163, 136, 0.12));
	box-shadow: 0 0 0 2px rgba(127, 70, 192, 0.3), 0 0 20px
		rgba(127, 70, 192, 0.35), 0 12px 32px rgba(63, 34, 100, 0.22);
...
.role-content {
	...
	border: 2px solid #e0d5f0;
	border-radius: 16px;
	background: linear-gradient(145deg, rgba(255, 255, 255, 0.95),
		rgba(251, 247, 255, 0.9));
...
.role-content::before {
	...
	position: absolute;
	inset: 0;
	background: radial-gradient(circle at 100% 0%, rgba(127, 70, 192, 0.08),
		transparent 60%),
		radial-gradient(circle at 0% 100%, rgba(44, 163, 136, 0.06),
		transparent 60%);
	opacity: 0;
...
.role-content::after {
	...
	inset: 0;
	border-radius: 14px;
	background: repeating-linear-gradient(45deg, transparent, transparent 2px, rgba(255,
		255, 255, 0.4) 2px, rgba(255, 255, 255, 0.4) 4px);
	opacity: 0;
	transition: opacity 340ms var(--ease-smooth);
	pointer-events: none;
}
...
.role-card:hover .role-content {
	border-color: #d0c4e8;
	background: linear-gradient(145deg, rgba(255, 255, 255, 0.98),
...
.role-icon-wrapper {
	...
	place-items: center;
	flex: none;
	background: linear-gradient(135deg, rgba(127, 70, 192, 0.15),
		rgba(44, 163, 136, 0.1));
...
.role-icon-wrapper::before {
	...
	inset: -2px;
	border-radius: 14px;
	background: linear-gradient(135deg, rgba(127, 70, 192, 0.3),
		rgba(44, 163, 136, 0.2));
	opacity: 0;
...
.password-strength {
	...
	padding: 0.85rem;
	border-radius: 13px;
	background: linear-gradient(135deg, rgba(127, 70, 192, 0.08),
		rgba(44, 163, 136, 0.06));
	border: 1px solid rgba(127, 70, 192, 0.2);
	box-shadow: 0 4px 12px rgba(127, 70, 192, 0.08), inset 0 1px 0
...
.password-strength::before {
	...
	position: absolute;
	inset: 0;
	background: radial-gradient(circle at 100% 0%, rgba(127, 70, 192, 0.1),
		transparent 60%);
	pointer-events: none;
}
...
.strength-bar {
	height: 8px;
	background: linear-gradient(90deg, rgba(220, 53, 69, 0.2),
		rgba(255, 152, 0, 0.15), rgba(76, 175, 80, 0.15));
	border-radius: 999px;
	overflow: hidden;
...
.strength-fill {
	...
	background: linear-gradient(90deg, #dc3545 0%, #ff9800 40%, #ffc107 70%, #4caf50
		100%);
	transition: width 320ms var(--ease-smooth), box-shadow 320ms
		var(--ease-smooth);
...
.completion-wrap {
	display: grid;
	gap: 0.5rem;
	background: linear-gradient(135deg, rgba(127, 70, 192, 0.04),
		rgba(44, 163, 136, 0.03));
	padding: 0.8rem;
	border-radius: 12px;
...
.progress {
	height: 12px;
	background: linear-gradient(90deg, rgba(220, 53, 69, 0.12),
		rgba(255, 152, 0, 0.1), rgba(76, 175, 80, 0.12));
	border-radius: 999px;
	overflow: hidden;
	position: relative;
...
.progress span {
	...
	box-shadow: 0 0 8px rgba(127, 70, 192, 0.4), inset 0 1px 0
...
.btn {
	...
	gap: 0.5rem;
	transition: transform 260ms var(--ease-smooth), box-shadow 260ms
		var(--ease-smooth), background-color 260ms var(--ease-smooth), filter
		260ms var(--ease-smooth);
	position: relative;
	overflow: hidden;
	letter-spacing: 0.03em;
	text-transform: uppercase;
...
.btn-primary {
	...
	box-shadow: 0 12px 28px rgba(63, 34, 100, 0.35), inset 0 1px 0
...
.btn-primary::after {
	...
	inset: 0;
	border-radius: 12px;
	background: radial-gradient(circle at 100% 0%, rgba(255, 255, 255, 0.3),
		transparent 60%);
	opacity: 0;
	transition: opacity 260ms var(--ease-smooth);
	pointer-events: none;
...
.btn-primary:hover {
	box-shadow: 0 16px 36px rgba(63, 34, 100, 0.45), inset 0 1px 0
...
.form-signature {
	...
	background: linear-gradient(135deg, rgba(127, 70, 192, 0.08),
		rgba(44, 163, 136, 0.06), rgba(201, 133, 71, 0.05)), #fffdff;
	padding: 0.95rem 1rem;
	position: relative;
	overflow: hidden;
	box-shadow: 0 12px 32px rgba(53, 31, 84, 0.12), inset 0 1px 0
		rgba(255, 255, 255, 0.8);
}
...
.form-signature::before {
	...
	position: absolute;
	inset: 0;
	background: radial-gradient(circle at 100% 50%, rgba(127, 70, 192, 0.1),
		transparent 60%);
	pointer-events: none;
}
...
.form-signature::after {
	...
	inset: 0;
	transform: translateX(-110%);
	background: linear-gradient(95deg, rgba(255, 255, 255, 0),
...
.signature-mark {
	...
	box-shadow: 0 14px 32px rgba(53, 30, 85, 0.32), inset 0 1px 0
		rgba(255, 255, 255, 0.3), 0 0 20px rgba(127, 70, 192, 0.25);
...
<body>
			...
			<nav class="nav-section" aria-label="Primary navigation">
              ...
              <path
					...
					<span class="nav-icon" aria-hidden="true"> <svg
							viewBox="0 0 24 24">
              <path
								d="M16 19v-1a4 4 0 00-8 0v1M12 11a3 3 0 100-6 3 3 0 000 6zM21 13a1 1 0 01-1-1v-2a1 1 0 012 0v2a1 1 0 01-1 1z" />
            </svg>
		...
		<main class="main reveal" data-reveal data-delay="140">
			<div class="topbar">
				...
				<div class="topbar-actions">
					<a
						href="${pageContext.request.contextPath}/DashboardController?action=viewDashboard"
						class="ghost-link"> <svg viewBox="0 0 24 24"
							style="width: 16px; height: 16px; stroke: currentColor; stroke-width: 2; fill: none; stroke-linecap: round; stroke-linejoin: round;">
              <path d="M19 12H5M12 19l-7-7 7-7" />
            </svg> Back to Dashboard
					</a>
					<div class="welcome-pill">
						<span class="pill-icon" aria-hidden="true">📚</span> Library Hub
					</div>
				</div>
			</div>

			<div class="content">
				<article class="card form-card no-top-orb reveal" data-reveal data-delay="280">
					...
					<c:if test="${empty user or empty user.userId}">
						<div class="flash-message flash-error">
							<span>User not found. <a
								href="${pageContext.request.contextPath}/UserController?action=showUserList">Back
									to Users</a></span>
						</div>
					</c:if>
					<c:if test="${not empty user and not empty user.userId}">
						<form id="userForm" class="user-form"
							...
							<div class="form-section">
								...
								<div class="field-grid two-col">
									<div class="field">
										<span> <svg viewBox="0 0 24 24" class="field-icon">
                      ...
                      <circle cx="12" cy="7" r="4" />
										...
										</span> <input type="text" id="lastName" name="lastName"
                      ...
                      <path
										...
										</span> <input type="tel" id="phone" name="phoneNo"
                    ...
                    <circle cx="12" cy="10" r="3" />
							...
							<div class="form-section">
								...
								<div class="role-grid">
									<label class="role-card"> <input type="radio"
										name="role" value="member" required
										<c:if test="${user.role eq 'member'}">checked</c:if> />
                      ...
                      <circle cx="12" cy="7" r="4" />
										...
										name="role" value="librarian"
										<c:if test="${user.role eq 'librarian'}">checked</c:if> />
										<div class="role-content">
											<svg viewBox="0 0 24 24" class="role-icon">
                      <path d="M4 19.5h16M6 12.5h12M4 5.5h16" />
                      <circle cx="12" cy="12.5" r="8" fill="none"
													stroke-width="2" />
                    </svg>
											<div>
							...
							<div class="form-footer">
								<div class="completion-wrap">
									<div class="completion-label">
										<strong>Form Completion</strong> <span id="completionPercent">0%</span>
									</div>
									<div class="progress">
                  ...
                  <path
								...
								<div class="signature-copy">
									...
									<blockquote>Register new library members to access
										our comprehensive collection and services.</blockquote>
								</div>
							</div>
						</form>
					</c:if>
    ...
    function initializeForm() {
      ...
      form.addEventListener('submit', function(e) {
        if (passwordField && confirmPasswordField && credentialsSection && credentialsSection.style.display !== 'none') {
